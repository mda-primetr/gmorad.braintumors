...
# Get metadata for stool and tumor samples ----
df_metadata_stool_tumor <- df_metadata %>%
    filter(microbiome_sample_type %in% c("Stool", "Tumor")) %>%
    mutate(sample_id = gsub("\\.", "-", sample_id)) %>%
    dplyr::select(sample_id, coded_id, microbiome_sample_type, tumor_category)

# Get stool and tumor samples that are matched ----
df_matched <- read_tsv(
    "data/raw_data/WGS/TumorStoolGenomeFilteredComparison.31Taxa.processedSame.percFiltered.origReconstructions.static.clusters.04.11.2025.txt"
) %>%
    janitor::clean_names() %>%
    dplyr::rename("Stool" = "other_sample", "Tumor" = "tumor") %>%
    inner_join(df_metadata_stool_tumor, by = c("Stool" = "sample_id")) %>%
    inner_join(df_metadata_stool_tumor, by = c("Tumor" = "sample_id")) %>%
...
df_plt_overlap <- df_matched %>%
    ...
    write_csv(
        "output/figures/WGS/matched_percentage_taxa_stool_tumor_alternative.csv"
    ) %>%
    mutate(coded_id_tumor = paste(coded_id, tumor_category, sep = "_"))


# Plot the data ----
plt_stool_overlap <- df_plt_overlap %>%
    ggplot(
        .,
        aes(
    ...
    labs(fill = "Species shared between tumor and stool") +
...
ggsave(
    filename = "output/figures/WGS/matched_percentage_taxa_stool_tumor_alternative.pdf",
    width = 12,
    height = 8
)


save(plt_stool_overlap,
    file = "output/figures/WGS/matched_percentage_taxa_stool_tumor_alternative.RData"
)
