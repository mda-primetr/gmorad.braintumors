...
signature_genes_df <- read_csv("Input_files/SMI/Glioma/glioma_tumor_gene_list.csv")

# Filter DGE to include only signature genes with significant upregulation
glioma_dge_all_sig <- glioma_dge_all %>%
...
# Set order
glioma_dge_all_sig$Category <- factor(
  glioma_dge_all_sig$Category,
  levels = c(
...
ggplot(glioma_dge_all_sig,
       aes(
         x = Patient,
         y = reorder(target, Order),
         fill = p.value,
         # Fill controls inner color
         size = log2FC
       )) +
  geom_point(shape = 21,
  ...
  facet_grid(
    ...
    scales = "free",
    space = "free_x",
    labeller = labeller(Category = label_wrap_gen(20))
  ) +
  coord_flip() +
...
# Save plot
ggsave(
  filename = "Output_files/SMI/Figures/Figure5B1.pdf",
  width = 10,
  height = 1.8
)
