...
Glioma_target_Data_BIS <- subset(Glioma_target_Data_BIS, !fData(Glioma_target_Data_BIS)$TargetName
    %in% BIS_probes_to_be_removed)

# Calculate geoMean of 16S probes (score)
BIS_probe_geoMean <- data.frame(assayDataApply(Glioma_target_Data_BIS,
...
# Create df with BIS scores and phenoData
df_assay_BIS <- pData(Glioma_target_Data_BIS) %>%
    mutate(Type = factor(Type, levels = c("Glioma", "Adjacent normal")))
...
# Add ROI # to BIS
pData(Glioma_target_Data_BIS) <- pData(Glioma_target_Data_BIS) %>%
    mutate(ROI = protocolData(Glioma_target_Data_BIS)$roi)
# Add ROI # to WTA
pData(Glioma_target_Data_WTA) <- pData(Glioma_target_Data_WTA) %>%
    mutate(ROI = protocolData(Glioma_target_Data_WTA)$roi)
