...
# Log samples
assayDataElement(object = BrM_target_Data_WTA, elt = "log_q_bgsub") <-
    assayDataApply(BrM_target_Data_WTA, 2, FUN = log, base = 2, elt = "q_norm_bgsub_1")

# Remove Tumor border
BrM_target_Data_WTA <- BrM_target_Data_WTA[, which(BrM_target_Data_WTA$Type != "Tumor border")]
...
pData(BrM_target_Data_WTA) <- pData(BrM_target_Data_WTA) %>%
    mutate(
        ...
        test_16SrRNA_status_25_75 = factor(pData(BrM_target_Data_WTA)$`16SrRNA_status_25_75`,
            levels = c("High", "Low")
        ),
...
df_BIS_scores <- as.data.frame(pData(BrM_target_Data_WTA)) %>%
    mutate(test_16SrRNA_status_25_75 = factor(test_16SrRNA_status_25_75,
        levels = c("Low", "High")
    )) %>%
...
for (compartment in "BrM") {
    ind <- pData(BrM_target_Data_WTA_16S_25_75)$Type == compartment
    mixedOutmc <-
...
BrM_DGE_by_16S_25_75 <- as.data.frame(results) %>%
...
# Plot genes
ggplot(BrM_DGE_by_16S_25_75_dotplot, aes(
